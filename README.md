...
Бот обратной связи представляет из себя связанную систему из тг-бота и тг-группы. В тг-бота пользователи пишут сообщения. Тг-группа — это «коллцентр», в который тг-бот будет пересылать сообщения пользователей и через который админы/редактция будет отправлять им ответы. На каждого написавшего в тг-бота бот создает в тг-группе отдельную тему.

Бот имеет возможности:
- диалог с каждым пользователем в отдельной теме
...
- админ может удалять сообщения пользователя в чате у юзера (командой /delete в тг-группе)
- админ может удалять сообщения свои в чате у юзера (командой /delete в тг-группе)
...
7. В тг-группу можно добавить других людей, которые будут ответственны за ответы на вопросы пользователей из бота. Протестируйте работу всех функции через тг-бота и тг-группу
    ...
    * проверьте, что вы можете редактировать ответ (просто редактируете ранее отправленное сообщение в тг-группе)
    * проверьте, что вы можете банить/разбанить конкретного пользвателя (команды /ban и /unban в тг-группе)
    * проверьте, что вы можете удалять свои сообщения, сообщения пользователя или все сообщения у пользователя (команды /delete и /delhistory в тг-группе)
