...
Бот обратной связи представляет из себя связанную систему из тг-бота и тг-группы. В тг-бота пользователи пишут сообщения. Тг-группа — это «коллцентр», в который тг-бот будет пересылать сообщения пользователей. Админы взаимодействуют с пользователями через тг-группу, через нее админы отправлют ответы пользователям.
...
- админ может удалять сообщения свои в чате у юзера (reply-ответом на конкретное сообщение с командой /delete в тг-группе)
