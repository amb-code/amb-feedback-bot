...
Бот обратной связи представляет из себя тг-бота и тг-группой с темами, связанных друг с другом. В тг-бота пользователи пишут сообщения, а админы/редакция через тг-группу отвечают им. Группа — это «коллцентр», в который тг-бот будет пересылать сообщения пользователей и через который админы/редактция будет отправлять им ответы. На каждого написавшего в тг-бота бот создает в тг-группе отдельную тему, в которой будет диалог только с этим человеком.
