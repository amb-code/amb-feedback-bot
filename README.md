...
Мы старались сделать ее настолько простой, насколько это возможно, чтобы команды, которые 
...
[Beget](https://beget.com). На момент написания самый дешевый сервер стоил 219 
рублей в месяц.

1. Пройдите регистрацию, создайте аккаунт и зайдите в контрольную панель 
   [здесь](https://beget.com/ru). После входа слева вы увидите кнопку 
...
4. Затем в разделе `Authentication` нужно будет развернуть пункты `Set password`
...
5. Готово, нажимаем кнопку `Create virtual server`. Слева на панели появился 
   ...
   Обратите внимание на кнопки `Terminal` и `File manager`, они нам понадобятся 
   на следующем шаге. 
...
4. Создадим файл окружения для `Docker Compose`. Это нужно, чтобы докер смог 
   найти нужную версию бота. Последнюю версию бота вы всегда можете проверить на 
...
6. Теперь создадим папку `env`, в которой будут лежать уже переменные самого 
...
6. Создадим сами файлы с переменными. Если вы на Windows, откройте стандартный 
   ...
   TG_BOT_USERNAME=<имя, которое вы выбрали при регистрации бота>
   ``` 

   Теперь создадим `db.env`:
   ```dotenv
...
7. Сейчас мы загрузим эти файлы в папку `env`, которую создали выше. В шаге 5 
   раздела 
   [Аренда сервера](#%D0%90%D1%80%D0%B5%D0%BD%D0%B4%D0%B0-%D1%81%D0%B5%D1%80%D0%B2%D0%B5%D1%80%D0%B0) 
   мы обращали внимание на кнопки `Terminal` и `File manager`. 
   Настало время нажать на кнопку `File manager`. Откроется файловый менеджер с доступом к диску 
...
9. Теперь запустим бот и проверим, все ли в порядке
   ...
   Если в конце логов бот говорит `Bot ready, listening...`, значит вы все 
