...
5. Готово, нажимаем кнопку `Create virtual server`. Слева на панели появился 
   ...
   Обратите внимание на кнопки `Terminal` и `File manager`, онаи нам понадобится 
   на следующем шаге. 
...
который мы записали на прошлом шаге. Приготовьтесь, это самая муторная часть.
...
2. Теперь нужно установить Docker. Поскольку команды в нем набирать вручную 
   тяжело, мы загрузим скрипт установки через файловый менеджер.

   Нажмите на кнопку `File manager`, откроется вкладка браузера с вот таким 
   окном.

   ![file_manager](./docs/file_manager.png)

   - В любой панели, левой или правой, зайдите в папку `root`.

   - Теперь скачайте себе на рабочий стол файл. Для этого переходим по ссылку,
     жмем на три точечки вверху справа и выбираем `Download`:
     [install_docker.sh](./scripts/manual_install/install_docker.sh)

   - Загрузим файл на сервер. Для этого в окне файлового менеджера вверху окна
     выберите кнопку `Загрузить файлы`. Теперь с рабочего стола вашего 
     компьютера загрузите файл, который только что скачали.

     Если вы все правильно сделали, вы увидите что-то такое:

     ![upload_inst_docker](./docs/upload_inst_docker.png)

   - Теперь время вернуться в терминал в предыдущей вкладке браузера (мы ушли 
     оттуда, когда открыли файловый менеджер) и выполнить там скрипт установки.
     Мы выполним две команды: дадим разрешение на выполнение файлу скрипта и 
     запустим сам скрипт. Вводите по одной строчке и запускайте.
     ```
